...
function Sidebar({ town, currentTab, onTab, stats, onOpenTowns, onOpenSettings }) {
  ...
  return (
    <aside className="ac-sidebar">
      ...
      <div className="ac-sidebar-foot">
        ...
        <button className="ac-foot-link" onClick={onOpenSettings}>Settings</button>
      </div>
    </aside>
  );
}
...
function ProgressMeter({ stats, gameId }) {
  const hasSea = (gameId === "acnh" || gameId === "acnl") && stats.sea && stats.sea.total > 0;
  const segKeys = hasSea
    ? ["fish","bugs","fossils","art","sea"]
    : ["fish","bugs","fossils","art"];

  const total = segKeys.reduce((a,k) => a + stats[k].total, 0);
  const done  = segKeys.reduce((a,k) => a + stats[k].donated, 0);
  const pct = total > 0 ? Math.round((done/total)*100) : 0;

  return (
    <div className={`ac-meter ${hasSea?"ac-meter-5":"ac-meter-4"}`}>
          ...
          return (
            <div key={k} className="ac-meter-seg" style={{flex: frac}}>
              ...
              <div className="ac-meter-seg-label">
                ...
                <span className="ac-meter-seg-name">{k}</span>
