...
**Status:** In implementation — Phases 1–10 shipped to `development`; ready to tag v0.9.0-beta  
...
### Phase 10 — Mobile Responsive Verification ✅ shipped (PR #80)

**Branch:** `feature/v09-phase-10-mobile`  
**Goal:** Audit and fix any mobile regressions; verify all 980px breakpoint behaviors.

**Checklist:**
- [x] Sidebar stacks above main at 980px (not hidden)
- [x] TownManager renders as bottom sheet at ≤720px
- [x] ProgressMeter 5-segment wraps to 2 rows at ≤980px; 4-segment unchanged
- [x] Settings sections collapse at ≤700px; danger buttons full-width
- [x] All touch targets ≥44px (nav items, donate toggle, glyph tile, drawer close, row chevron)
- [x] Hero headline wraps without overflow at 390px
- [x] Scroll-to + highlight works on mobile (identical behavior per Decision 10 spec)
- [x] Category section headers + row meta readable at 390px
- [x] GlobalSearchDropdown keyboard nav inaccessible without hardware keyboard on mobile — tap-to-select via onClick handlers; kbd hint footer hidden ≤720px
