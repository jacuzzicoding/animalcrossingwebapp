...
- **"+" (add town) button did nothing when the dropdown was open** — `TownSwitcher` rendered a `fixed inset-0 z-10` backdrop while the dropdown was open to catch outside clicks, but that backdrop sat above the `+` and edit buttons (which had no z-index). Taps on `+` hit the backdrop first, so its `setOpen(false)` fired and the button's own `onClick` never ran — the create-town modal never opened. The prior two attempts on this branch adjusted modal-side rendering (iOS Safari zoom, `isOpen` gate) but not the upstream tap-swallow. Fixed by replacing the backdrop with a ref + `document.addEventListener('mousedown')` outside-click pattern (matching `useSearch.ts`), and hardening the `+` / edit / toggle buttons with explicit `type="button"` and `touchAction: 'manipulation'` to eliminate the iOS Safari double-tap delay.
